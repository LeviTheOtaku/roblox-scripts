...
local w = library:CreateWindow("Rocket v" .. version)
w:Section("Crates")
local selected = "none"
w:Dropdown("Box Types", {
flag = "boxtype";
list = {
"Inferno";
"Unreal";
...
w:Toggle("Auto Open Crates", {flag = "autocrates"})
spawn(function()
while wait(7.5) do
if w.flags.autocrates == true then
if selected ~= "none" then
if game.Players.LocalPlayer.Crates[selected].Value >= 1 then
game.ReplicatedStorage.MysteryBox:InvokeServer(selected)
end
end
end
end
...
w:Button("Buy Blueprints", function()
...
w:Section("Automation")
w:Toggle("Auto Rebirth", {flag = "autorebirth"})
spawn(function()
while true do
wait(1)
if w.flags.autorebirth == true then
game.ReplicatedStorage.Rebirth:InvokeServer()
end
end
end)

w:Toggle("Auto Remote", {flag = "autoremote"})
spawn(function()
while true do
wait()
if w.flags.autoremote == true then
game.ReplicatedStorage.RemoteDrop:FireServer()
end
end
end)

w:Toggle("Auto Button", {flag = "autobutton"})
spawn(function()
while true do
wait(0.1)
if w.flags.autobutton == true then
...
w:Toggle("Auto Loadout 1", {flag = "autoloadout1"})
spawn(function()
while true do
wait(2)
if w.flags.autoloadout1 == true and #game.Players.LocalPlayer.PlayerTycoon.Value:getChildren() -4 ~= #game:GetService("HttpService"):JSONDecode(game.Players.LocalPlayer.Layouts.Layout1.Value) then
local tycoon = game.Players.LocalPlayer.PlayerTycoon.Value
game.ReplicatedStorage.Layouts:InvokeServer("Load","Layout1")
end
end
end)
w:Toggle("Auto Loadout 2", {flag = "autoloadout2"})
spawn(function()
while true do
wait(2)
if w.flags.autoloadout2 == true and #game.Players.LocalPlayer.PlayerTycoon.Value:getChildren() -4 ~= #game:GetService("HttpService"):JSONDecode(game.Players.LocalPlayer.Layouts.Layout2.Value) then
local tycoon = game.Players.LocalPlayer.PlayerTycoon.Value
game.ReplicatedStorage.Layouts:InvokeServer("Load","Layout2")
end
end
end)
w:Toggle("Auto Loadout 3", {flag = "autoloadout3"})
spawn(function()
while true do
wait(2)
if w.flags.autoloadout3 == true and #game.Players.LocalPlayer.PlayerTycoon.Value:getChildren() -4 ~= #game:GetService("HttpService"):JSONDecode(game.Players.LocalPlayer.Layouts.Layout3.Value) then
local tycoon = game.Players.LocalPlayer.PlayerTycoon.Value
...
w:Toggle("Auto Star Loop", {flag = "autostarloop"})
spawn(function()
local tycoon = game.Players.LocalPlayer.PlayerTycoon.Value

workspace.DroppedParts[tycoon.Name].ChildAdded:connect(function(ore)
if not ore:findFirstChild("Fuel") then
if w.flags.autostarloop then
workspace.CurrentCamera.CameraSubject = ore
local cash = ore:WaitForChild("Cash")
if ore:FindFirstChild("Cash") then

if tycoon:findFirstChild("Catalyzed Star") then
repeat
if ore.Cash.Value < 1e+30 then
wait(0.65)
if tycoon:findFirstChild("Morning Star") then
wait(0.25)
ore.Velocity = Vector3.new(0, -1, 0)
ore.CFrame = tycoon["Morning Star"].Model.Upgrade.CFrame
end
end
if not ore:findFirstChild("NovaStar") and tycoon:findFirstChild("Nova Star") then
ore.Velocity = Vector3.new(0, -1, 0)
ore.CFrame = tycoon["Nova Star"].Model.Upgrade.CFrame
wait(0.4)
else
wait(0.3)
end
...
ore.Velocity = Vector3.new(0, -1, 0)
ore.CFrame = tycoon["Catalyzed Star"].Model.Upgrade.CFrame
wait(0.4)
until ore.Cash.Value > 1e+60

for i=1,2 do
local itemsParts = tycoon:getDescendants()
for i=1,#itemsParts do
if itemsParts[i].Name == "Upgrade" then
if not string.match(string.lower(itemsParts[i]:GetFullName()), "star") and not string.match(string.lower(itemsParts[i]:GetFullName()), "tesla") and not string.match(string.lower(itemsParts[i]:GetFullName()), "tracker") then
print(tostring(ore.Cash.Value) .. " from " .. itemsParts[i]:GetFullName())
ore.Velocity = Vector3.new(0, -1, 0)
ore.CFrame = itemsParts[i].CFrame
wait(0.1)
end
end
end
...
for i=1,#itemsParts do
if itemsParts[i].Name == "Upgrade" then
if string.match(string.lower(itemsParts[i]:GetFullName()), "tesla") then
print(tostring(ore.Cash.Value) .. " from " .. itemsParts[i]:GetFullName())
ore.Velocity = Vector3.new(0, -1, 0)
ore.CFrame = itemsParts[i].CFrame
wait(0.1)
...
local furnace = tycoon:getChildren()
for i=1,#furnace do
if furnace[i]:findFirstChild("Model") then
if furnace[i].Model:findFirstChild("Lava") then
ore.CFrame = furnace[i].Model.Lava.CFrame
end
end
end
repeat wait() until ore ~= nil
wait(1)
game.ReplicatedStorage.Rebirth:InvokeServer()
wait(4)
game.ReplicatedStorage.Layouts:InvokeServer("Load","Layout1")
...
w:Toggle("Auto Box Farm", {flag = "boxfarm"})
spawn(function()
while wait(0.1) do
if w.flags.boxfarm == true then
local crates = workspace:getChildren()
for i=1,#crates do
if crates[i]:findFirstChild("BoxModule") then
crates[i].Anchored = true
crates[i].CanCollide = false
crates[i].Transparency = 1
crates[i].Size = Vector3.new(50000,50000,50000)
end
end
local crates = workspace.Shadows:getChildren()
for i=1,#crates do
if crates[i]:findFirstChild("BoxModule") then
crates[i].Anchored = true
crates[i].CanCollide = false
...
w:Button("TP ores to player", function()
local tycoon = game.Players.LocalPlayer.PlayerTycoon.Value
...
w:Toggle("TP ores to furnace", {flag = "tporestofurnace"})
spawn(function()
local plrbase = game.Players.LocalPlayer.PlayerTycoon.Value
workspace.DroppedParts[plrbase.Name].ChildAdded:connect(function(ore)
if w.flags.tporestofurnace == true then
local lava = plrbase:getDescendants()
for i=1,#lava do
if lava[i].Name == "Lava" then
ore.CFrame = lava[i].CFrame + Vector3.new(math.random(-1,1), math.random(-1,1), math.random(-1,1))
...
w:Toggle("Anti AFK", {flag = "antiafkkick"})
spawn(function()
while true do
wait(1) 
if w.flags.antiafkkick == true then
...
w:Bind("Toggle Gui", {
flag = "toggleopenclosemain";
kbonly = true;
default = Enum.KeyCode.X;
}, function()
local window = nil
local windowFinder = game.CoreGui:getDescendants()
for i=1,#windowFinder do
if windowFinder[i].Name == "Rocket v" .. version then
window = windowFinder[i]
end
end
...
w:Section("Created by Levi#7059")
w:Section("LeviTheOtaku @ v3rm")


local w2 = library:CreateWindow("Universe TP")
w2:Section("Universe Teleports")
local pages = game:GetService("AssetService"):GetGamePlacesAsync()
while true do
for _,place in pairs(pages:GetCurrentPage()) do
local b = w2:Button(place.Name, function()
game:GetService("TeleportService"):Teleport(place.PlaceId, game.Players.LocalPlayer)
end)
end
...
local windowFinder = game.CoreGui:getDescendants()
for i=1,#windowFinder do
if windowFinder[i].Name == "Universe TP" then
window = windowFinder[i]
end
end
if window.Visible == true then
